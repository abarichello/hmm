...
{
	public class TeamKillUpdater : IMissionProgressUpdater
	{
		public TeamKillUpdater(IPlayerStats playerStats, IMatchStats matchStats, Mission mission)
		{
		...
		private float HandleRedTeam(IPlayerStats playerStats, IMatchStats matchHistory, float progressModifier)
		{
			...
			{
				return (float)matchHistory.GetTotalKillsRed() * progressModifier;
			}
			return (float)matchHistory.GetTotalKillsRed();
		}
		...
		private float HandleBlueTeam(IPlayerStats playerStats, IMatchStats matchHistory, float progressModifier)
		{
			if (playerStats.MatchWon)
			{
				return (float)matchHistory.GetTotalKillsBlue() * progressModifier;
			}
			return (float)matchHistory.GetTotalKillsBlue();
		}
