...
{
	public class TeamBombCarrierKillUpdater : IMissionProgressUpdater
	{
		public TeamBombCarrierKillUpdater(IPlayerStats playerStats, IMatchStats matchStats, Mission mission)
		{
		...
		private float HandleRedTeam(IPlayerStats playerStats, IMatchStats matchHistory, float progressModifier)
		{
			...
			{
				return (float)matchHistory.GetTotalBombCarrierKillsRed() * progressModifier;
			}
			return (float)matchHistory.GetTotalBombCarrierKillsRed();
		}
		...
		private float HandleBlueTeam(IPlayerStats playerStats, IMatchStats matchHistory, float progressModifier)
		{
			if (playerStats.MatchWon)
			{
				return (float)matchHistory.GetTotalBombCarrierKillsBlue() * progressModifier;
			}
			return (float)matchHistory.GetTotalBombCarrierKillsBlue();
		}
